...
final_tally_category <- raw_df$category_learning %>% 
  ...
  dplyr::rename(category_learning = "total_bonus_tally")

final_tally_size <- raw_df$size %>% 
  subset(!is.na(size_final_tally)) %>% 
  dplyr::select(PID, size_final_tally) %>% 
  dplyr::rename(size = "size_final_tally")

final_tally_rl <- raw_df$rl %>% 
  subset(!is.na(total_bonus_tally)) %>% 
  dplyr::select(PID, total_bonus_tally) %>% 
  dplyr::rename(rl = "total_bonus_tally")
...
# Run diagnostics for mturk participants 
...
```
# Make sure experiment works ok

```{r}

# ------------------------------------------------------------------------------
# 1. make sure repetition of exemplars happens only in old trials 
# ------------------------------------------------------------------------------

left_stims <- df$category_learning %>% subset(is_practice == 0 & no_response == 0) %>% dplyr::select(PID, left_category, left_exemplar, old_trial, repeated_trial) %>% dplyr::rename(category = "left_category", exemplar = "left_exemplar")
right_stims <- df$category_learning %>% subset(is_practice == 0 & no_response == 0) %>% dplyr::select(PID, right_category, right_exemplar, old_trial, repeated_trial) %>% dplyr::rename(category = "right_category", exemplar = "right_exemplar")
all_stims <- bind_rows(left_stims, right_stims)

# if there is no repetition of trials, we get 15 exemplars per category shown for novel trials. 
all_stims %>% 
  group_by(PID, old_trial, category) %>%
  dplyr::summarise(n = n())

# no repetition of exemplars 
all_stims %>% subset(repeated_trial==0) %>% group_by(PID, old_trial, category, exemplar) %>%
  dplyr::summarise(n = n())

# ------------------------------------------------------------------------------
# 2. look at distribution of delta values 
# ------------------------------------------------------------------------------

ggplot(df$category_learning %>% subset(old_trial==0), aes(x=delta_cumulative_value)) + 
...
ggplot(df$category_learning %>% subset(old_trial==0), aes(x=left_category_value - right_category_value)) + 
  geom_histogram(binwidth=.9) +
  facet_wrap(.~PID)
  
# ------------------------------------------------------------------------------
# 3. check how long each block takes (and make sure it is the same length for all participants and all blocks) - do this for category learning, size and rl 
# ------------------------------------------------------------------------------
...
block_duration <- df$events_log %>% 
  ...
  dplyr::summarise(length_duration = sum(duration),
                   length_duration_time_elapsed = sum(duration_time_elapsed))
# it seems the last trial of the last block in the category learning task doesn't include a feedback and iti. 

# ------------------------------------------------------------------------------
# 4. check how many trials each block includes in the category learning task
# ------------------------------------------------------------------------------

df$category_learning %>% group_by(PID, block) %>%
  dplyr::summarize(n= n())

# ------------------------------------------------------------------------------
# 5. look at delta value for size and rl
# ------------------------------------------------------------------------------

ggplot(df$size, aes(x=delta_size)) + 
  geom_histogram() +
  facet_wrap(.~PID)

ggplot(df$rl, aes(x=delta_cumulative_value)) + 
  geom_histogram() +
  facet_wrap(.~condition)

prop_rl <- df$rl %>%
  mutate(acc = ifelse(high_group_on_left == left_chosen, 1, 0)) %>%
  group_by(PID, condition) %>%
  dplyr::summarize(p_optimal = mean(acc, na.rm=1))

# ------------------------------------------------------------------------------
# 6. decide on suitable percentage of bonus money 
# ------------------------------------------------------------------------------

percentage <- 0.00055
df$category_learning %>%
  group_by(PID, block) %>% 
  dplyr::summarize(chosen_earnings = sum(chosen_exemplar_value, na.rm=1) * percentage,
                   max_earnings = sum(ifelse(left_exemplar_value>right_exemplar_value, left_exemplar_value, right_exemplar_value), na.rm=1) * percentage,
                   min_earnings = sum(ifelse(left_exemplar_value<right_exemplar_value, left_exemplar_value, right_exemplar_value), na.rm=1) * percentage) 

percentage <- 0.00075
df$rl %>%
  group_by(PID, block) %>% 
  dplyr::summarize(chosen_earnings = sum(chosen_item_value, na.rm=1) * percentage,
                   max_earnings = sum(ifelse(left_item_value>right_item_value, left_item_value, right_item_value), na.rm=1) * percentage,
                   min_earnings = sum(ifelse(left_item_value<right_item_value, left_item_value, right_item_value), na.rm=1) * percentage) 

# ------------------------------------------------------------------------------
# 7. Check how many old trials appear in every block
# ------------------------------------------------------------------------------

test <- df$category_learning %>% 
  mutate(within_cat_test = ifelse(left_category == right_category, 1, 0)) %>%
  subset(repeated_trial == 0 & within_cat_test==1) %>%
  group_by(PID, block) %>%
  dplyr::summarize(n = n())
